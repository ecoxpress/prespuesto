...
function init_date() {
  ...
  var intwoweeks = new Date(now.getTime() + 3 * 24 * 60 * 60 * 1000);
  ...
  var twoweeks =
    day + "-" + month + "-" + intwoweeks.getFullYear().toString().substr(2, 2);
...
function calculate() {
  ...
  $(".invoicelist-body tbody tr").each(function () {
    ...
    let amount = parseFloat(row.find(".amount input").val());

    if (isNaN(rate) || isNaN(amount)) {
      console.error("Invalid input data. Rate and amount must be numeric.");
      return;
    }
  ...
  const total = total_price + total_tax;
